...
test.describe("User management app tests.", async () => {
  ...
  test("TL-14-1 create user test", async ({ request }) => {
    ...
    const createdUser = UserDTO.serializeResponse(
      await creationResponse.json(),
    );
  ...
  test("TL-14-2 find user test", async ({ request }) => {
    ...
    const searchResponse = await request.get(
      `http://localhost:3000/users/${createdUser.id}`,
    );
  ...
  test("TL-14-3 delete user test", async ({ request }) => {
    ...
    const deletedUser = UserDTO.serializeResponse(
      (await deleteResponse.json())[0],
    );
