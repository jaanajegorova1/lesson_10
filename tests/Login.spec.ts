...
test.describe("Login tests", async () => {
  ...
  test("TL-12-2 Successful authorization and order creation", async ({
    request,
  }) => {
    const apiClient = await ApiClient.getInstance(request);
    const responseCreateOrder = await request.post(
      `https://backend.tallinn-learning.ee/orders`,
      {
  ...
  test("TL-12-3 order status", async ({ request }) => {
    ...
    const requestedOrder = OrderDto.serializeResponse(
      await responseOrderStatus.json(),
    );
